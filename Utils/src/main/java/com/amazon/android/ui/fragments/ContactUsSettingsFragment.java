...
/**
 * Provides fragment to be shown when "Contact Us" settings item is clicked.
 */
public class ContactUsSettingsFragment extends ASettingsFragment {
    ...
    public void createFragment(final Activity activity,
        ...
        commitFragment(manager, dialog, activity.getString(R.string.contact_us_settings_fragment_tag));
        mActivity = activity;
    ...
    private SingleViewProvider getSingleViewProvider(Context context) {
        ...
        try {
            content = Helpers.getContentFromFile(context,
                                                 context.getString(R.string.contact_us_file));
        }
        catch (Exception e) {
